...
int main(int argc, char* argv[])
{
    ...
    while ( !exit ) 
    {
      ...
      read_table (tab_brick, Tab, &x, &y, row, col, &nbI, &nbII, &nbIII, &nbIV, &nbV, &nbVI, screen);
    
      while (!gameover && !finish_level)
      {
	//look if the level is finished  
	finish_level = (nbI == 0) && (nbII == 0) && (nbIII == 0) && (nbV == 0) && (nbVI == 0);   
	...
	* else we move the ball */
	...
	collide_table (tab_brick, &nbI, &nbII, &nbIII, &nbIV, &nbV, &nbVI, &ball, screen -> format, NB_BRICKS);  
