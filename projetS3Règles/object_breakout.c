...
void handle_event_menu (SDL_Event event_menu, int *exit, int *begin, SDL_Surface *screen, SDL_Surface *background) {
...
void read_table (object_breakout **table, char **tab_data, float *x, float  *y,
		 int row, int col, int *nbI, int *nbII, int *nbIII, int*nbIV, int *nbV, int *nbVI,
		 SDL_Surface *screen) {
  int i,j;
  for (i=0; i<row; i++) {
    for (j=0; j<col; j++) {
	...
	*nbIV += 1;
	...
	table[4][*nbV] = init_object (5, *x, *y, screen);
	*x += table[4][*nbV].width;
	*nbV += 1;
      }
      else if ( tab_data[i][j] == '6' ) {
	table[5][*nbVI] = init_object (6, *x, *y, screen);
...
void collide_table (object_breakout ** table, int *n, int *m, int *l, int *p, int *q, int *r, object_breakout *ball, SDL_PixelFormat *format, int nbtype) {
...
void menu (int *exit, SDL_Surface *screen, SDL_Surface *background) {
  ...
  while (!begin)
  {
    ...
    if (SDL_PollEvent(&event_menu)) {
      handle_event_menu (event_menu, exit, &begin, screen, background);
    }
      draw_background (background, screen);
      print_text ("Breakout", -1, screen, SCREEN_WIDTH / 8, SCREEN_HEIGHT / 3, 150);
      print_text ("Press the m bottom for see the rule menu", -1, screen, SCREEN_WIDTH / 8,( SCREEN_HEIGHT / 3) + 150, 20);
...
void print_name (SDL_Surface *screen, SDL_Surface *background, char *name)
{
    ...
    while (!end) {
        ...
        print_text ("Quel est votre nom ? ", -1, screen, SCREEN_WIDTH / 3, 2, 32);
        print_text ("Win", -1, screen, SCREEN_WIDTH / 3, SCREEN_HEIGHT / 3, 150);
        print_text ("Press the return key for replay or escape for exit", -1, screen, (SCREEN_WIDTH / 4)   ,  (2*SCREEN_HEIGHT) / 3, 20);
...
void ranking (SDL_Surface *screen, SDL_Surface *background, int score_player) 
{
  ...
  if (file != NULL) {
    ...
    sc_t = atoi (score_t);
    printf("second %s\n",second);
    if ( (score_player > sc_f) || strcmp(first,"# \n") ) {
      printf("1\n");
    ...
    } else if ( (score_player > sc_s) || strcmp(second, "# \n") ) {
      printf("2\n");
        sprintf(third, "%s", second);
    ...
    } else if ( (score_player > sc_t) || strcmp(third,"# \n") ) {
      printf("3\n");
    ...
    print_text("1 : ", -1, screen, (SCREEN_WIDTH / 2) - 50, (2 * SCREEN_HEIGHT / 3) + 50, 20);
    print_text(first, sc_f, screen, SCREEN_WIDTH / 2,  2 * SCREEN_HEIGHT / 3 + 50, 20);
    print_text("2 : ", -1, screen, (SCREEN_WIDTH / 2) - 50,  2 * SCREEN_HEIGHT / 3 + 85, 20);
    print_text(second, sc_s, screen, SCREEN_WIDTH / 2,  2 * SCREEN_HEIGHT / 3 + 85, 20);
    print_text("3 : ", -1, screen, (SCREEN_WIDTH / 2) - 50,  2 * SCREEN_HEIGHT / 3 + 120, 20);
    print_text(third, sc_t, screen, SCREEN_WIDTH / 2,  2 * SCREEN_HEIGHT / 3 + 120, 20);
...
void rule (SDL_Surface *screen, SDL_Surface *background)
{
    ...
    draw_background(background, screen);
    print_text("Rules", -1, screen, SCREEN_WIDTH/3, SCREEN_HEIGHT/3, 64);
    print_text ("Les briques bleues et jaune se detruisent en 1 coup", -1, screen, SCREEN_WIDTH/5,  SCREEN_HEIGHT/2, 20);
    print_text ("Les briques vertes et beiges se detruisent en 2 coup", -1, screen, SCREEN_WIDTH/5, (SCREEN_HEIGHT/2) + 40, 20);
    print_text ("Les briques rouges se detruisent en 1 coup", -1, screen, SCREEN_WIDTH/5, (SCREEN_HEIGHT/2) + 70, 20);
    print_text ("Les briques noires ne se detruisent pas", -1, screen, SCREEN_WIDTH/5, ( SCREEN_HEIGHT/2) + 100, 20);
    print_text ("Le but du jeu est de detruire toutes les briques sauf les noires.", -1, screen, SCREEN_WIDTH/5, ( SCREEN_HEIGHT/2) + 130, 20);SDL_UpdateRect (screen, 0, 0, 0, 0);
    print_text ("Pour lancer la balle appuyer sur espace", -1, screen, SCREEN_WIDTH/5, (SCREEN_HEIGHT/2) + 160, 20);
    print_text ("La plateforme se deplace de gauche a droite a l'aide des fleches", -1, screen, SCREEN_WIDTH/5, (SCREEN_HEIGHT/2 ) + 190, 20);
    print_text ("Pour revenir au menu, appuyer sur echap", -1, screen, SCREEN_WIDTH/5, ( SCREEN_HEIGHT/2) + 230, 20);
    SDL_UpdateRect (screen, 0, 0, 0, 0);
