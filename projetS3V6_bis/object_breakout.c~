...
bool collide_precise (object_breakout *sprite1, object_breakout *sprite2, SDL_PixelFormat *format, int *cu, int *cv) {
  ...
  if (test) {
	...
	*bmp_it++ = *sprite_it;
      ...
      sprite_it += (sprite2->sprite->w - sprite2->width);
    }
...
bool collide_table_aux (object_breakout *table , int *n, object_breakout *ball, SDL_PixelFormat *format) {
  ...
  while (!exit2 && i < *n) {
    if (collide_precise (ball, &table[i], format, &u, &v)) {
      if (table[i].y <= (ball -> y + ball -> height)) {
	rebond (ball, 1);
	exit2 = true;
      }      
      else if (table[i].x + table[i].width >= (ball -> y)) {
	printf("aur");
	rebond (ball, 2);
	exit2 = true;
      }
      else if (table[i].x <= (ball -> x + ball -> width)) {
	printf("bon");
	rebond (ball, 0);
	exit2 = true;
      }
      else if (table[i].y + table [i].height >= (ball -> y)) {
	rebond (ball, 3);
	exit2 = true;
      }
...
void collide_platform (object_breakout *plat, object_breakout *ball, SDL_PixelFormat *format){
  ...
  if (collide_precise(ball, plat, format, &cx,&cy)){
    if (ball -> x < plat -> x + plat -> width){ 
      if (plat -> y <= cy){
	printf("b");
	ball -> speed_y =  1.5;
      }
    }
    else if (ball -> x + ball -> width/2 < ball -> x){
      if (plat -> y <= cy){
	printf("a");
	ball -> speed_y = -1.5;
      }
