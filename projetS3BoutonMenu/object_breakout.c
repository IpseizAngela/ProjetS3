...
void read_table (object_breakout **table, char **tab_data, float *x, float  *y,
  ...
  for (i=0; i<row; i++) {
	...
	*nbVI += 1;
	...
	table[6][*nbVII] = init_object (7, *x, *y, screen);
...
void collide_table (object_breakout ** table, int *n, int *m, int *l, int *p, int *q, int *r, int *s, object_breakout *ball, SDL_PixelFormat *format, int nbtype) {
...
void rule (SDL_Surface *screen, SDL_Surface *background)
{
    ...
    print_text ("* Les briques bleues et jaune se detruisent en 1 coup", -1, screen, x,  y, 32, 0);
    print_text ("* Les briques vertes et beiges se detruisent en 2 coup", -1, screen, x, y + 40, 32, 0);
    print_text ("* Les briques rouges et roses se detruisent en 3 coup", -1, screen, x, y + 80, 32, 0);
    print_text ("* Les briques noires ne se detruisent pas", -1, screen, x, y + 120, 32, 0);
    print_text ("* Le but du jeu est de detruire toutes les briques ", -1, screen, x, y + 160, 32, 0);
    print_text ("sauf les noires.", -1, screen, x + 50, y + 200  , 32, 0);
    print_text ("* Pour lancer la balle appuyer sur espace", -1, screen, x, y + 240, 32, 0);
    print_text ("* La plateforme se deplace de gauche a droite", -1, screen, x, y + 280, 32, 0);
    print_text (" a l'aide des fleches.", -1, screen, x + 50, y + 320, 32, 0);
    print_text ("Pour revenir au menu, appuyer sur echap", -1, screen, 2, y + 380, 60, 0);
    print_text ("sur echap ou entree", -1, screen, x + 100, y + 440, 60, 0);
    SDL_UpdateRect (screen, 0, 0, 0, 0);
