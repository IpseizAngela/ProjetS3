...
void handle_event_lose (SDL_Event event_lose, int *exit, int *choose) {
...
void lose (int *exit, SDL_Surface *screen, SDL_Surface *background,FILE *file) {
   int choose = 0;
   while (!choose) {
     SDL_Event event_lose;
     if (SDL_PollEvent (&event_lose)) {
       handle_event_lose (event_lose, exit, &choose);
     }
     draw_background (background, screen);
...
void win (int *exit, SDL_Surface *screen, SDL_Surface *background, FILE *file) {
  int choose = 0;
  while (!choose) {
    SDL_Event event_win;
    if (SDL_PollEvent (&event_win)) {
      handle_event_lose (event_win, exit, &choose);
    }
    draw_background (background, screen);
