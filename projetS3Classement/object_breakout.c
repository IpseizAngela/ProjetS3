...
void print_name (SDL_Surface *screen, SDL_Surface *background)
{
    ...
    while (!end) {
        ...
        if (SDL_PollEvent (&event_name)) {
            handle_event_name (event_name, &end, &n, tab_name);
        }
        draw_background (background, screen);
        ...
        print_text ("Press the return key for replay or escape for exit", -1, screen, (SCREEN_WIDTH / 4)   ,  (2*SCREEN_HEIGHT) / 3, 20);
        for (i = 0; i < n; i++) {
            printf("tab_name[%d] = %c, name = %c \n", i, tab_name[i], name);
            //strcat(name, tab_name[i]);
            printf("1 nme %c\n", name);
        }
        if (n>0) {
            printf("2 nme %c\n", name);
            print_text(tab_name[0], -1, screen, x, 50, 32);
        }
...
{
    FILE *file_ranking = fopen("ranking.txt", "r+");
