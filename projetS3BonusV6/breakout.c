...
/****** Il faut regarde pourquoi avec le bonus cross la vie devient 0 au bout d'un moment ***********/
/****** J'ai regardé, c'est le troisième élément du tableau qui pose problème, quand je veux supprimer l'élément 
 qui est couramment le premier, il fait une erreur de segmentation quand il doit déplacer le troisième élément ******/
...
int main(int argc, char* argv[])
{
    ...
    while ( !exit ) 
    {
      ...
      /*for(int zi=0;zi<row;zi++){
          for(int zj=0;zj<col;zj++){
              printf(" %c ",Tab[zi][zj]);
          }
    }*/
      int i;
	...
	* else we move the ball */
	if (!throw) {
	  ...
	  move_ball (ball, &nb_ball, &platform, tab_bonus,screen, &throw, &nb_bonus);
	}
